...
# codespace 创建过程
  ...
  通过存储库创建codespace，由于在克隆仓库之后才创建了 codespace 的 dev 容器，因此在 dev 容器映像中配置的任何 git 模板目录（隐藏目录）将不适用于你的 codespace。 在创建代码空间后，必须在 `devcontainer.json` 配置文件中添加 `Git LFS` 的feature，然后执行 `rebuild container`。  
