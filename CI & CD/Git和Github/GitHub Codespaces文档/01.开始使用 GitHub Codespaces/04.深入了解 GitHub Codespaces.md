...
### 步骤 2：创建开发容器
  ...
  > [!NOTE]
  > 通过存储库创建codespace，由于存储库在创建容器之前已克隆到主机 VM 上，因此 git 模板目录（隐藏目录）中的任何内容都不会应用于 codespace，
  > 除非你在 `devcontainer.json` 配置文件中添加 `Git LFS` 的feature，然后执行 `rebuild container`。  
