<img width="1233" height="651" alt="image" src="https://github.com/user-attachments/assets/81b57b9e-ba36-442c-b36b-f90370bfc692" />Docker 架构是基于客户端-服务器模式的，其中包括多个关键组件，确保容器化应用的高效构建、管理和运行。  
...
# Docker 架构示意图
  <img width="1233" height="651" alt="image" src="https://github.com/user-attachments/assets/4a5e43c4-a78f-4f98-bace-75fa90e38a99" />  

# Docker 架构的工作流程
  - 构建镜像：使用 Dockerfile 创建镜像。
  - 推送镜像到 Docker Registry ：将镜像上传到 Docker Hub 或 Docker Registry 中。
  - 拉取镜像：通过 docker pull 从 Docker Registry 中拉取镜像。
  - 运行容器：使用镜像创建并启动容器。
...
## Docker 守护进程（Docker Daemon）
  ...
  - 管理容器的网络和存储。
  - 启动、停止、查看容器日志等。
  - 与 Docker  Docker Registry 进行通信，管理镜像的存储与分发。
...
## Docker 镜像（Docker Images）
  ...
  Docker 镜像可以通过 docker pull 从 Docker Hub 或私有 Docker Registry 拉取，也可以通过 docker build 从 Dockerfile 构建。  
  拉取 Ubuntu 镜像：  
  ```bash
  docker pull ubuntu
  ```
