# 概念
  ...
  为了让大家更好地感‌受到抽象工厂模式的作用，以跨平台U⁢I组件库为例，我们需要支持不同操作‏系统（Windows、Mac）下的‍按钮、文本框等UI组件。  
  让我们来看⁡看使用和不使用抽象工厂模式的区别：  

  <img width="1902" height="1030" alt="image" src="https://github.com/user-attachments/assets/b7a809f7-c80e-438b-88ca-79c28d8dc6f5" />

# 基本结构
  1. 抽象工厂（AbstractFact‌ory）：声明一组创建产品的方法。                     ——产品族工厂接口
  2. 具体工厂（ConcreteFac⁢tory）：实现创建具体产品对象的方法。                 ——具体产品族工厂类
  3. 抽象产品（Abstract‏Product）：定义每个产品的公共接口。                     ——产品接口
  4. 具体产品（Concret‍eProduct）：实现具体的产品对象。                        ——具体产品类
  5. 业务端（Client）：只⁡依赖抽象工厂和抽象产品，负责调用工厂去生产对象。

  <img width="1434" height="612" alt="image" src="https://github.com/user-attachments/assets/021b4bb1-2df8-4c9b-8ba8-2ff86ba3a02a" />

# 代码实现
  下面就以 ‌“多平台 UI 渲⁢染” 为例，我们用‏抽象工厂模式实现一‍个简单的组件系统。  
