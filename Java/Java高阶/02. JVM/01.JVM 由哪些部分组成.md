...
# 类加载器子系统（ClassLoader）
  ...
  类加载器的主要过程包括：  
  - 加载（Loading）：找到类的字节码(.class 文件)，并以 JVM 内部规定的格式把它存起来，同时创建一个访问入口。  
  ...
  - 链接（Linking）：将类文件的数据合并到 JVM 中，分为验证（Verification）、准备（Preparation）和解析（Resolution）三个阶段。  
    ...
    - 解析 (Resolution) - "查地址"阶段:将元空间中的运行时常量池中的符号引用（一个名字）替换为直接引用（一个具体的地址或句柄）。  
...
# 运行时数据区（Runtime Data Area）
  详见[Java/Java高阶/02. JVM/02.JVM 的内存区域(Runtime Date Area)是如何划分的.md](https://github.com/jinlemon7/learning_Display/blob/c8dfdd70bef5ee795dacb4638f231b5d67797b13/Java/Java%E9%AB%98%E9%98%B6/02.%20JVM/02.JVM%20%E7%9A%84%E5%86%85%E5%AD%98%E5%8C%BA%E5%9F%9F(Runtime%20Date%20Area)%E6%98%AF%E5%A6%82%E4%BD%95%E5%88%92%E5%88%86%E7%9A%84.md)

# 执行引擎（Execution Engine）
  执行引擎负责将字节码转化为机器指令并执行  
  Interpreter和Just-In-Time Compiler在整个Rumtime Date Area上工作：  
  - 解释器执行指令寻址：  
    Program Counter Register 
  - 解释器执行方法调用：  
    VM Stack 的 Stack Frame  
  - 解释器执行类信息（类的结构信息）、静态变量（只依赖类、不依赖对象实例的变量）、运行时常量池的读取操作  
  - JIT编译器执行JIT编译后的代码缓存 ：
  - 垃圾回收器执行卸载类/回收运行时常量池中的废弃常量
    MetaSpace  
  - 解释器和JIT编译器执行对象数据的读取和修改：
  - 垃圾回收器执行垃圾回收：  
    Heap  
  - 解释器和JIT编译器执行JNI调用：  
    Native Method Stack  
  
  1. 解释器(Interpreter)：逐行解释字节码并执行（一边解释一边执行），适用于程序首次运行时
  2. JIT编译器(Just-In-Time Compiler)：将热点代码（频繁执行的代码）编译为机器码存储，提升执行效率
  3. GC(Garbage Collector)垃圾回收器：详见[Java/Java高阶/02. JVM/03.Java 中有哪些垃圾回收算法.md](https://github.com/jinlemon7/learning_Display/blob/c8dfdd70bef5ee795dacb4638f231b5d67797b13/Java/Java%E9%AB%98%E9%98%B6/02.%20JVM/03.Java%20%E4%B8%AD%E6%9C%89%E5%93%AA%E4%BA%9B%E5%9E%83%E5%9C%BE%E5%9B%9E%E6%94%B6%E7%AE%97%E6%B3%95.md)

# 本地方法接口（Native Interface，简称 JNI）
  允许Java调用非Java代码（c/c++）,便于与操作系统和其他本地库进行交互。  
  JNI提供跨语言调用能力，Java程序可以访问操作系统级别的功能和高性能库。  

# JVM组成框架图
  <img width="1711" height="2708" alt="image" src="https://github.com/user-attachments/assets/034d9777-4194-4630-b88e-add00784354a" />
