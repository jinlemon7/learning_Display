...
## 方法引用和等效Lambda
  ...
  作用域：可访问外部变量（Lambda表达式内的局部变量需为final修饰）  
  ```java
  int count = 0;               
  names.forEach( name->{
...
# 匿名类
  ...
  {} 的作用： 可重写父类/接口方法或添加新成员（字段/方法）从而创建一个继承自父类/接口的实例  
...
# List/Array的初始化
  ...
  | Array.asList(...) | 半可变 | 快速创建支持set()的列表（set是覆盖），不支持增删 |  
  ...
  List.of()和Arrays.asList()只能初始化List类型引用，初始化Arraylist/LinkedList类型引用会报错。  
