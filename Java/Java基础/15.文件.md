...
# 文件和目录路径-Path、Paths类
  ...
  ------
  - isAbsolute()：是否是绝对路径
  - getFileName()：返回相对路径的文件名
  - getParent()：返回绝对路径的前缀
  - getRoot()：返回根目录
  - toRealPath()：转换为绝对路径
  - toUri()：转换为Uri
  - toFile()：转换为File
  - startsWith()：比较路径组件是否开始于······
  - endsWith()：比较路径组件是否终止于······
  ...
  public class Pathinfo {
    ...
    public static void main(String[] args) {
      ...
      File f = ap.toFile();         // 不要被骗了,本质上是一个Path
    }
  }
  ```
  ```java
...
## 选择Path的片段-Path::getNameCount()、for-in语法
  ```java
...
## 添加或删除路径片段-Path::relativize()、Path::resolve()
...
# 目录-Files工具类、Files.walkFileTree()、Files.walk()
...
# 文件系统-FileSystems工具、FileSystems.getDefault()
...
# 监听 Path-WatchService、Path::register(WatchService, 事件)、FileSystems.getDefault().newWatchService()
  ```java
  import java.io.IOException;
  import java.nio.file.*;
  ...
  */
  ```
  - delTxtFiles()中的try块看上去是冗余的.因为它们都在捕捉相同类型的异常。所以似乎有外部的try就足够了。然而，由于某些原因(可能是个bug ). Java要求两者都存在
...
# 查找文件-PathMatcher、FileSystems.getDefault().getPathMatcher()
...
# 读写文件-Files.readAllLines(Path)、Files.lines(Path)、Files.write(Path, Iterable)
  ...
  */
  ...
  ```
  - 因为我们是在同一个块中执行的所有操作，所以两个文件可以在相同的try-with-resources块中打开
