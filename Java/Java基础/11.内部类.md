...
# 创建内部类
  ...
  将对象的类型指定为`OuterClassName.InnerClassName`  
...
# 匿名内部类
  ...
  ```
  - 将适当的参数传给基类构造器
  - 匿名类内部要用到匿名类之外定义的对象，参数引用要用final修饰，尽管x被传给了匿名类的基类构造器，但是在该匿名类内部，它并没有被直接使用到
...
## 接口中的类
  ...
  ```
  这会生成一个叫TestBed$Tester的独立的类（要运行这个程序，可以执行`java TestBed$Tester`）  
...
## 闭包与回调
  ...
  内部类是面向对象的闭包，因为它不仅包含外围类对象（“ 被创建时所在的作用域”）的每一条信息，  
...
# 内部类与控制框架
  ...
  public class Controller {
    ...
    public void run() {
      ...
      //创建一个副本，这料在选择列表中的元素就不会改动列表了
      for(Event e : new ArrayList<>(eventList))      //匿名类
      if(e.readyO) {
...
# 内部类可以被重写吗
  ...
  public class BigEgg2 extends Egg2 (
    public class Yolk extends Egg2.Yolk {            //显式继承内部类
      public Yolk() (
        System.out. printIn ("BigEgg2. Yolk ()");
      }
      @Override
      public void f() {
        System.out .printin("BigEgg2. Yolk. f ()");
      }
    }
  ...
  BigEgg2.Yolk.f()
  ```
  insertYolk()方法允许BigEgg2将它的Yolk对象向上转型为Egg2中的y
  所以当g()调用y.f()时，用到的是f()的重写版本  

# 局部内部类
  局部内部类不能使用访问权限修饰符，因为它不是外围类的组成部分，  
  但是它可以访问当前代码块中的常量(final变量或实际为final)，以及外围类中的所有成员  
  ```java
  public class LocalInnerClass {
    ...
    Counter getCounter(final String name) {
      ...
      class LocalCounter implements Counter {
        ...
        @Override
        public int next() {
          System.out .print (name);             // 访问局部的 final 变量
          return count++;
        }
      }
    ...
    Counter getCounter2(final String name) {
      return new Counter() {
        //匿名类不能有其名的构造器，只有一个实例初始化部分
        {
          System.out.println("Counter()");
        }
        @Override
        public int next() {                 // 重载版本
          System.out.print (name);         // 访问局部 final 变量
          return count++；
        }；
      }
      public static void main(String[] args) {
        LocallnnerClass lie = new LocalInnerClass();
        Counter
        cl = lie.getCounter("Local inner “)，
        c2 = lie.getCounter2("Anonymous inner ");
        for(int i = 0; i < 5; i++)
          System.out.println(cl.next());
        for(int i = 0; i < 5; i++)
          System.out.printin(c2.next());
      }
    }
  }
  ```
  局部内部类允许我们定义具名的构造器，而匿名类只能使用实例初始化(不能有构造器)   

# 内部类标识符
  文件/类的命名遵循一个公式：外围类的名字，加上$再加上内部类的名字  
  如果内部类是匿名的，编译器会以数字作为内部类标识符   
  如果内部类嵌套在其他内部类之内，它们的名字会被附加到其外围类标识符和$之后  
  LocalInnerClass.java 创建的.class 文件包括：  
  ```java
  Counter.class
  LocalInnerClass$1.class
  LocallnnerClass$LocalCounter. class
  LocallnnerClass.class
  ```

  为了让内部类正常工作，Java编译器会以各种各样的方式修改你的内部类  
