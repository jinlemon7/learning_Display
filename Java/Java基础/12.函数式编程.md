...
# 方法引用
  ...
  */
  ```
  - [1]show()的签名(参数类型和返回类型)和Callable中call()的签名兼容
  - [2]我们将Describe对象的一个方法引用赋值给了一个Callable, Callable中没有show()方法，  
    只有一个call()方法 然而，Java似乎对这种看似奇怪的赋值并没有意见，因为这个方法引用的签名和Callable中的call()方法兼容  
...
# 未绑定方法引用
  ...
  */
  ```
  引用的方法，与其关联接口方法(方法引用所实现的接口方法)的签名是兼容(参数、返回类型相同)的  
...
# 函数式接口
  一个函数（无论是实例方法、static方法、内部类的方法、匿名内部类的方法、lambda表达式、方法引用）与函数式方法的签名兼容，那么就可以作为该函数式方法所在函数式接口的具体实现。  
  签名兼容：两个函数的参数列表和返回类型一致
...
# java.util.function  
  ...
  | 两个参数的 Consumer: 第一个参数是引用.第二个参数是基本类型 | ObjtypeConsumer accept() | ObjIntConsumer<T></br> ObjLongConsuner<T></br> ObjDoubleConsumer<T> |  
  ...
  | 一个参数；返回boolean | Predicate test() | Predicate<T></br> IntPredicate</br> LongPiedicate</br> DoublePredicate |  
...
# 高阶函数
  ...
  */
  ...
  public class TransformFunction {
    static Function<I,O> transform(Function<I,O> in) {  //4.in是步骤2的transform()中定义的lambda表达式
...
# 函数组合
  ...
  | compose(argument)先执行参数操作.再执行原始操作 | Function</br> UnaryOperator</br> IntUnaryOperator</br> LongUnaryOperator</br> DoubleUnaryOperator |  
  | and(argument)对原始谓词和参数谓词执行短路逻辑与(AND)计算 | Predicate</br> BiPredicate</br> IntPredicate</br> LongPredicate</br> DoublePredicate |  
  | or(argument)对原始谓词和参数谓词执行短路逻辑或(OR)计算 | Predicate</br> BiPredicate</br> IntPredicate</br> LongPredicate</br> DoublePredicate |  
...
# 柯里化和部分求值
  ...
  */
  ```
  1. [1]Function中有另一个Function作为其第二个参数(返回值)
  2. [2]柯里化的目的是能够通过提供一个参数来创建一个新函数，所以我们现在有了一个“参数化函数”(原函数已经传入一个参数，生成了剩下的子函数)和剩下的“自由参数”(还未确定的参数)
  ...
  Function<String,
    Function<String,
      Function<String, String>>> sum =
        a -> b -> c -> a + b + c;
  ```
  对于每一层级联箭头，我们都要在其  函数式接口类型  声明外再包裹另一个Function。  
