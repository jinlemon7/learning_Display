...
## 跟踪与调试: peek()
  ...
  */
  ```
  - peek(System.out::print)并不会立刻打印，而是在forEach(System.out::print)打印时才会随着遍历的流元素，按照管线的peek()顺序打印————惰性求值原则
  - peek()接受的是一个遵循Consumer函数式接口的函数，这样的函数没有返回值.所以也就不可能用不同的对象替换掉流中的对象

## 对流元素进行排序: sorted(...) 
  默认的比较方式：sorted()  
  还有一种接受Comparator参数(可以是兼容的lambda)的soited()形式  
  ```java
...
## 移除元素: distinct(), filter(Predicate)
  ...
  ```
  - rangeClosed()包含了上界
  - 如果没有任何一个取余操作的结果为0,则noneMatch()操作返回tiue。如果有任何一个计算结果等于0,则返回false。
...
## 在应用map()期间解除流和组合流: flatMap(Function), flatMapTotype(...), typeStream.concat(Stream1, Stream2)
  ...
  */
  ...
  - 因为iand.ints()会生成一个IntStream,所以这里必须使用flatMap(). concat()和of()的特殊的Integer版本
...
# Optional类型: 特殊流元素类型, Stream调用标准的流操作返回Optional对象
  ...
  - reduce()的一个版本，会将返回值包在一个Optional中
...
# 终结流操作(消费流元素): 由Stream<T>调用
  作接受一个流，并生成一个最终结果。不会再把任何东西发给某个后端的流。  
...
## 在每个流元素上应用某个终结操作: forEach(Consumer)，forEachOrdered(Consumer)
  ...
  551 589861 555 288 128 429 207 693 200 258 522 868 961
  258 555 693 861 961 429 868 200 522 207 288 128 551 589
   */
  ```
...
# 组合所有的流元素: reduce(BinaryOperator), reduce(identity, BinaryOperator)
  ...
  static Frobnitz supply() {
    return new Frobnitz(rand.nextlnt(BOUND));
  }
  ...
  - 只有当结果不是empty时，Optional.ifPresent()方法才会调用Consumer<Frobnitz> (之所以printin能够符合，是因为它可以通过toString()将Fiobnitz转化为一个String)
...
## 选择一个元素
  ...
  */
  ```
  - reduce()的参数(即(nl, n2) -> n2 是用两个元素中的后一个替换了这两个，这样最终得到的就是流中的最后一个元素了
...
## 获得流相关的信息
  ...
  System.out.println(
    FileToWords.streamC'Cheese.dat")
      .max(String.CASE_INSENSITIVE,ORDER)
      .orElse("NONE"));
  ```
  ```java
  /*输出
  ...
  */
  ...
  ```java
  System.out.println(rands().averageO.getAsDouble());
  System.out.println(rands().max().getAsIntO);
  System.out.printin(rands() .min().getAsIntO);
