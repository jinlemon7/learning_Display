...
## 异常参数
  标准异常处理程序都有两个构造器
  1. 无参构造器
  2. 有参构造器，接受一个String参数，在异常类中放置相关信息
  ```java
  if(t == null)
...
## try块
  ...
  类似C这样不支持异常处理的语言往往需要临时机制，在每个函数调用后直接放置错误测试代码，并立即检查结果。  
...
# 异常说明
  ...
  `void f() throws TooBig, TooSmall, DivZero{...}`    //throws关键字  
  如果无throws，则意味该方法不会抛出异常（Runtime Exception除外，这是可以在任何地方抛出而不需要说明的异常）  
  throws抛出的异常————这种在编译时被检查并强制实施的异常叫检查型异常（checked exception）  
  “向下转型”检查基于泛型，一样属于RuntimeException（运行时异常）  
...
# 捕捉任何异常
  ...
  Throwable fillInStackTrace()  返回记录栈帧当前状态信息（定位从当前方法开始）的Throwable对象，在重新抛出错误/异常时很有用。  
  PS：二次抛出当前异常位置的栈轨迹，覆盖原有异常对象，返回一个throwable对象  

  Throwable的基类Object有一些方法：  
  - getClass()          返回表示这个对象的类的class对象
  - getClass().getName()           返回包路径+类名
...
## 异常链（异常链抛出）
  ...
  - Error(JVM用来报告系统错误)、Exception、RuntimeException提供带Cause参数的构造器
  - 其他异常类使用initCause(cause)方法
...
# 标准Java异常
  ...
  异常定义在Java.lang.util/net/io等其他库，例如所有的IO异常都继承自Java.io.IOException  
...
# 异常的约束
  在继承体系结构中,重写方法只能抛出基类版本方法中说明的异常(及异常子类)范围中的异常  
...
# 异常发生在构造器
  ...
  最安全的用法是使用嵌套的try块（放在创建可能抛出异常的类对象的其他类中，清理放在成功执行创建后的try块中）  
  ```java
  class CleanUp{
...
### 构造可能会抛出异常并且需要清理的对象
  ...
  每个对象的构造操作（创建）后必须有一个try-finally语句块来保证清理, 处理"需要清理"  
...
# 异常匹配
  异常处理系统会按照处理程序的编写顺序来寻找“最近的”那个处理程序  
  当找到第一个匹配的处理程序时，他会认为异常已经得到了处理，从而不再进一步搜索  
  子类的对象可以匹配其基类的处理程序  
  向一个方法中加入更多的派生异常，只要客户程序员的代码捕捉的是基类异常，那就不需要修改  

# 其他可选方式
  检查型异常（强制编写异常处理catch子句）：强迫客户程序员添加catch子句，尽管我们可能未准备好错误处理。   
  ————导致“吞食有害”问题：严重的问题被空catch捕获，无法引起程序员重视  
...
## 检查型异常和静态类型检查对于稳健的程序开发来说是必不可少的
  ...
  2. 类型检查，不管什么时候进行。只要确保类型的正确使用，关于是在编译时还是运行时实施，这并不重要
...
## “检查型异常”转换为“非检查型异常”
  ...
  try{
    switch(type){
      case 0: throw new FileNotfoundException();          //throwRuntimeException()方法不会抛出任何检查型异常                               
      case 1: throw new IOException();                    //当我们准确捕捉异常时，将getCause()方法调用放在try块中取出包含的检查型异常并分别捕捉
      case 2: throw new RuntimeException("Where am I");
      default: return;
    }
  }catch(IOExcepton | FileNotfoundException e){
    throw new RuntimeException(e);                        //异常包入RuntimeException中
  }
  ```
  ```
  for( int i = 0; i < 3, i++){
...
# 异常使用指南
  ...
  2. 要在恰当的层次处理问题 （除非知道怎么处理，否则不要捕捉异）
  3. 可以使用异常来修复问题，并重新调用引发异常的方法                 ————重新调用引发异常的方法是可选的
  4. 可以选择做好补救措施然后继续，不再重新尝试引发异常的方法           ————是否重试是程序员的处理策略
  5. 可以借助异常处理的过程计算出某个结果，以代替该方法应该生成的值
  6. 可以在当前上下文中把能做的事情都做了，然后将相同的异常重新抛出，使其逬入更上层的上下文中
  7. 可以在当前上下文中把能做的事情都做了，然后重新抛出一个不同的异常，使其逬入更上层的上下文中
  8. 使用异常来终止程序
  9. 使用异常来简化问题 （如果你的异常模式使问题更复杂了，用起来会非常麻烦 ）
  10. 使用异常让我们的库和程序更安全 （这既是为调试所做的短期投资，也是为程序的稳健性所做的长期投资 ）

# 小结
  1. Java异常处理不到10%可以从错误中恢复。绝大部分只是将栈展开到一个一致已知的稳定状态，而不是真正执行了恢复行为
  2. Java坚持所有的错误都要以异常的形式报告，一致的错误报告系统，意味着不需要每写一段代码都要想想“会不会有错误成为漏网之鱼”。  
     只要你不会自己把异常吞掉，这是重点！  
