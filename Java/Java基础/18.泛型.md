...
由于java泛型使用类型擦除实现，很难编写真正足够应用于广泛类型的代码，除了：  
- 奇异递归泛型(curiously recurring generics, CRG )：基类变成了一种为其子类实现通用功能的模板,但是所实现的功能会将派生类型用于所有的参数和返回值
- 辅助潜在类型机制——未绑定方法引用：显式地提供要使用的未绑定方法引用作为Consumer、方法调用方对象P，实现代码适用于无限制的P
...
## 边界的行为
  ...
  */
  ```
  尽管kind看起来是被存储为`Class<T>`，但是类型擦除意味着它实际上只是被存为一个不带参数的Class  
  因此，在你使用它的时候，如同创建数组一样，Array.newInstance()并不实际掌握kind含有的类型信息。它无法生成具体的结果，因此必须进行类型转换(类型标记`Class<T>`会在类型擦除后恢复类型)，这会产生令人不悦的警告  
  ...
  ```
  - 尽管我们知道(由于类型擦除)，在create()内部的`new ArrayList<>()`方法中，`<T>`被移除了————在运行时，类中并没有`<T>`
...
## `Class<T>`创建类型实例
  ...
  */
  ```
  - 这样可以编译成功，但如果使用`ClassAsFactory<Integer>`则会失败，因为Integer中并没有无参构造器
...
## 泛型数组
  你是无法创建泛型数组的。通用的解决办法是不管在何处,你都用ArrayList来创建泛型数组：  
  ```java
...
为目标数组类型 这确实可以通过编译.但会在运行时抛出ClassCastException异常：  
  ...
  public class ArrayOfGeneric {
    ...
    public static void main(String[] args) {
      try {
        gia = (Generic<Integer>[])new Object[SIZE];             //[1]
      } catch(ClassCastException e) {
        System.out.println(e.getMessage());
      }
      //运行时的类型是原始类型(已被擦除)：
      gia = (Generic<Integer>[] )new Generic[SIZE];             //[2]
  ...
  */
  ```
  - [1]gia被转型为`Generic<Integer>[]`,该信息也只会存在于编译时。在运行时，它仍然还是Object数组，而这会导致问题
  - [2]唯一可以成功创建泛型类型数组的方法就是创建一个类型为被擦除类型的新数组，然后再对其进行类型转换
...
# 边界————多重边界、单边界
  ...
  不过如果能够将参数类型限制在某个类型的子类集合中，你就可以调用该基类上可用的方法了  
...
## 编译器有多聪明？————参数化类型向上转型的唯一方式
  ...
  ```
  - 在你声明了`ArrayList<? extends Fruit>`后,add()的参数便成了 `? extends Fruit`
  ...
  */
  ```
  - 如果创建了`Holder<Apple>`,就无法将其向上转型为`Holder<Fruit>`,但是可以向上转型为`Holder<? extends Fruit>`
  - 向上转型为`Holder<? extends Fruit>`后，如果调用get(),则只能返回Fruit
...
## 逆变性
  ...
  public class GenericReading {
    ...
    static void f2() {
      ...
      // 无法转型为List<Fruit>————Reader<Fruit>已确定参数类型为Fruit
    }
  ...
  ```
  - 静态泛型方法readExact()对毎个泛型调用都可以有效地兼容
  - 如你在门f1()中所见，从`List<Apple>`中返回Apple,以及从`List<Fruit>`中返回Fruit
  - 如果使用泛型类的话，在你为该类实例化一个对象的时候，其参数即被确定下来
  - 如f2()所示，因为fruitReader的具体类型是Fruit,所以它可以从`List<Fruit>`中读取单个Fruit。但是`List<Apple>`应该也能生成Fruit对象，而fruitReader并不允许这样
  - CovaiiantReader.readCovaiiant()方法接收了 `List<? extends T>`作为参数。从该list中读取T是安全的，因为你知道里面的所有元素都至少是T,也可能是T的某种子类。在f3()中，你可以看到现在可以从`List<Apple>`中读取Fruit了。

## 无界通配符————`?`/`? extends T`不能被set，可以get；`? super T`可以被set，不能get
  `?`给以赋值给确定参数类型  
  原生容器可以赋值给`?`,不能被赋值给`T`、`? extends T`、`? super T`  
  ...
  ```
  - 原生List不可以赋值给`List<? extends Object>`
  ...
  ```
  - `Map<?, ?>` 可以赋值给 `Map<确定参数类型, ?>`
  - 泛型参数会被擦除为其第一个边界类型,，`List<?>`会看起来等同于`List<Object>`,而List也实际上相当于`List<Object>`————只是这两种说法都并不完全正确
  - List实际上是指“持有任意Object类型的原生List”，而`List<?>`是指“持有某种具体类型的非原生List”，但我们并不知道是什么类型
  ...
  public class Wildcards {
    ...
    public static void main(String[] args) {
      ...
      Object r3 = exact1(unbounded);       // 必须返回 Object
  ...
  ```
  - `Holder<?>`和`Holder<? extends T>`不能被set(), `Holder<?>`的get()返回Object,`Holder<? extends T>`的get()返回T
  - 原生Holder不能set(),get()返回Object，`Holder<? super T>`可以set()T和T的子类，不能get()
  - 原生Holder不能赋值给`Holder<T>`、`Holder<? extends T>`、`Holder<? super T>`
  ...
  - Holder和`Holder<?>`两者是不同的。这是因为原生Holder可以持有任何类型的组合，而`Holder<?>`则只能持有由某种具体类型组成的单类型集合，因此你不能只传入一个 Object
...
因为可以使用基类的地方，由于多态性的缘故，就一定也可以使用派生类(因此使用T)。不过，调用get()的意义不大,因为holder持有的类型可以是任意超类，所以唯一安全的类型是Object
  ...
  - 如果你向某个以“具体的”泛型类型(无通配符)为参数的方法传入了原生Holder类型的引用.就会产生瞥告，因为T在原生类型中并不存在
  - exact2()所受的限制最大，因为它明确地需要传入`Holder<T>`和类型参数T。如果未给该方法指定具体的参数.就会产生错误或警告
...
## 基本类型不可作为类型参数
  Java泛型的限制之一，是你无法将基本类型作为类型参数，因此你无法创建例如`ArrayList<int>`这样的类型  
...
## 无法实现同一个泛型接口的两种变体
  ...
  ```
  - 因为类型擦除将`Payable<Employee>`和`Payable<Hourly>` 降级为相同的类Payable。所以上述代码意味着你会将同一个接口实现两次.因此Hourly是无法编译的
...
## 基类会劫持包含参数化类型的接口，子类无法实现更改了参数化类型的相同接口
  ```java
...
## 自限定
  ...
  */
  ...
  class B extends SelfBounded<A> {} // 这样也可以
...
## 反射
  ...
  */
  ```
  - 这两个类之间并无直接关联.也没有共同的基类(除了 Object)或接口
...
# Java 8中的辅助潜在类型机制——未绑定方法引用
  ...
  */
  ```
  - CommunicateA.perform()在无限制的P上被泛型化。它可以是任何类型，只要有供其可用的`Consumer<P>`即可————此处，那些`Consumer<P>`表示不带参数的P的方法的未绑定方法引用
...
## 使用Supplier的泛型方法
  ...
  - 由于该未绑定方法adder必须接收一个参数（要添加到holder中的元素），adder必须是一个`BiConsumer<H, A>`
