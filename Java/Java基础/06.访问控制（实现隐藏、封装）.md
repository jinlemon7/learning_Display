# package:包（库/库单元/命名空间）：避免不同库相同类名造成的冲突
  ...
  一个包并不会真正被“打包”到一个文件中，多个".java"文件（源代码-文本文件）显得混乱，所以设置默认将一个包下的".class"文件（字节码-二进制文件）放入一个目录中。（设置CLASSPATH路径到存放目录）  
...
# Java访问权限修饰符
  ...
  PS：protected和private不能修饰普通类（内部类除外）
...
# 类的访问权限
  ...
  类（内部类除外）不可用protected/private修饰，会导致其他所有类均无法访问（报错）。  
