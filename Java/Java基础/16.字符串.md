...
## 格式说明符
  ...
  - argument_index$: 位置索引标识符，用于显式指定该格式说明符要使用哪个参数的值。
    - 重复使用同一个参数（无需传多次），改变参数的使用顺序（格式化顺序 ≠ 参数传入顺序），有选择地使用部分参数（跳过某些参数）
    - 例:
    - ```
      // 英文日期格式：月/日/年
      System.out.printf("%2$d/%3$d/%1$d%n", 2024, 12, 25);
      // 输出：12/25/2024 （参数顺序被重新排列）
      ```
    - 一旦使用索引标识符，所有格式符都必须使用索引，否则抛出 MissingFormatArgumentException
  - flags: 用于控制输出格式的对齐方式、正负号显示、补零、千位分隔符等外观特性。
    | 标志 | 含义 | 适用类型 | 示例（`"%-10d"`, 参数 `123`） |  
    | :--- | :--- | :--- | :--- |  
    | `-` | **左对齐**（默认右对齐） | 所有类型 | `"123       "` |  
    | `+` | 始终显示正负号（正数也显示 `+`） | 数字、`BigDecimal` | `"+123"` |  
    | ` ` | 空格：正数前加空格（负数前仍是 `-`） | 数字、`BigDecimal` | `" 123"` |  
    | `0` | 用 `0` 填充空白（不能与 `-` 同时用） | 数字、`BigDecimal` | `"0000000123"` |  
    | `,` | 添加千位分隔符 | 整数、浮点数、`BigDecimal` | `"1,234,567"` |  
    | `(` | 负数用圆括号包裹 | 数字、`BigDecimal` | `"(123)"` 表示 `-123` |  
  - width: 控制一个字段的最小长度
...
## 基础
  ...
  数字原义用`\d`来描述，但是反斜杠在正则表达式中有特殊的作用  
  `\\`的意思是“我正在插入一个正则表达式反斜杠，所以后面的字符有特殊含义”  
  要表示一个数字，你的正则表达式字符串应该是`\\d`。要在正则表达式中插入普通的单反斜杠，你可以用`\\\\` 
  ...
  public class Backslashes {
    public static void main(String[] args) (
      String one =  "\\"；      // 反斜杠在String中有特殊的作用：转义，所以一个单反斜杠要用"\\"表示
      String two = "\\\\"；
      String three = "\\\\\\";
  ...
  */
  ...
  `(-|\\+)?`    
  - +字符在正则表达式中具有特殊意义.所以在表达式中插入一个+，必须用`\\`转义成普通字符
...
### 2. Matcher的group()
  ...
  - public String group(int i)传入前一次匹配操作结果期间要进一步匹配的分组号。如果compile()匹配成功，但指定的分组未能匹配前一次匹配操作结果的任何部分.则返回null
