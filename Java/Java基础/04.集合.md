...
# 泛型和钻石语法
  ...
  泛型（Generics）是 Java 5 引入的一种语法机制，用来在 类、接口、方法 中引入 类型参数。  
  ```java
  class Box<T> {                  //泛型类
    private T value;
  ...
  interface Repository<T> {                //泛型接口
      void save(T entity);
      T findById(int id);
  }
  ```

  不适用泛型<T>，会造成由于默认支持Object，所有对象都可以加入容器，但编译器不会报错，但使用容器中对象时报错。  
  指定类型 例：`ArrayList<Apple> apples = new ArrayList<>();`         //左侧已有Apple，右侧不需要再写  
...
# LinkedList (等同于Deque, 可作Queue, Stack)
  ```mermaid
  graph TD;
    尾部 --> x2;
    x2 --> x1;
...
# record类型
  ...
  1. record的字段是隐含private final修饰，所以不允许初始化后修改  
     定义的方法也不允许修改字段，只能读取
  2. record可以由其他对象/record组成  
     record隐含final，不能被其他类继承，也不能继承其他类
  3. record定义可镶嵌在类和局部方法中，隐含static修饰  
     record在类和方法中创建符号引用时，同样隐含static修饰，是一个类级别的常量引用  
     常量：record字段隐含private final，只有Getter访问器方法，没有Setter，基本类型和对象一经初始化后不可修改  
     ```java
     ...
     ```
  5. 除了自动构建构造器，程序员可使用紧凑构造器（compact constructor）初始化字段  
     例：  
     ```java
